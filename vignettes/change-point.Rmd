...
ggplot(data2) + theme_bw() + 
  geom_point(aes(x = avetemp, y = kwhd)) + 
  ggtitle("Change Point Example Data (Data from DHP)") +
  xlab("Average Outdoor Temperature (F)") +
  ylab("Daily kWh") + xlim(35, 70)
```
...
pf <- ggplot(df) + geom_line(aes(x = Ts, y = LLs)) + theme_bw() +
  ggtitle("Change Point Example Maximum Likelihood (Data from DHP)\nVertically Dashed Lines Indicate Data Points") +
  xlab("Change Point (F)") +
  ylab("Log Likelihood of Regression Model") + xlim(35, 70)
for(i in 1:nrow(data2)) {
  pf <- pf + geom_vline(xintercept = data2$avetemp[i], linetype = "dashed")
}
print(pf)
```
