...
vignette: >
  %\VignetteIndexEntry{Change Point}
...
The rub with the change point regression model for building energy use data is that it is not differentiable. What does that mean? Well, it most practically means that we cannot estimate coefficients and standard errors in the usual way. We also can't use likelihood ratio tests to assess significance of heating or cooling, which will be discussed more in the next section.

The two figures below show an example of what we mean by a non-differentiable regression model, specifically that the likelihood function is not "smooth". The first graphic shows energy vs temperature for an example dataset (in this case a home in the NW United States that has been retrofitted with a heat pump). 
...
pf <- ggplot2::ggplot(df) + 
  ggplot2::geom_line(ggplot2::aes(x = Ts, y = LLs)) + 
  ggplot2::theme_bw() +
