...
We can increase the smoothing with the "days" argument to set a wider rolling mean.
...
1. Request a token from NOAA https://www.ncdc.noaa.gov/cdo-web/token, and put the following line in your .Rprofile (or load it interactively).

```{r}
noaa_key <- "your_key_here"
```

2. Search for relevant stations with stationSearch
...
3. Choose the most appropriate station with stationCompare
...
4. Read the weather data with read.ghcn
