...
There's a function called "projection" that returns a projection object that you can print or plot. You can do it with either the change point or degree day models. The change point model will let you project through more data faster, but you can really only trust the total projected use, not the breakdown in to heating/cooling. The degree day model will be slower and use less data, but you can better trust the heating/cooling breakdown. Here's how it works, projecting the degree day model through the last 20 years of weather data.

```{r, fig.width=7,fig.height=5}
p <- projection(dd, "GHCND:USW00024233", nYears = 20)
p
